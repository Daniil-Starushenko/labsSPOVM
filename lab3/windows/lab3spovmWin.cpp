...
void Client() {
    ...
    HANDLE work = OpenSemaphore(SEMAPHORE_ALL_ACCESS, FALSE, "work"); //  открытие семафора, дооступ к нему
    HANDLE close = OpenSemaphore(SEMAPHORE_ALL_ACCESS, FALSE, "close"); // открытие семафора, доступ к нему
    ...
    LPVOID pointer = MapViewOfFile(fileProjection, FILE_MAP_ALL_ACCESS, 0, 0, 0); // получение указателя на данную область памяти
    // атрибуты:
    ...
    while (true) {
        ...
        if (WaitForSingleObject(close, constants::kDelay) == WAIT_OBJECT_0) {
            CloseHandle(work); // завершение дескриптора открытого объекта
            CloseHandle(close); // завершение дескриптора открытого объекта
...
void Server(char* input) {
    //создание семафоров
    HANDLE work = CreateSemaphore(NULL, 0, 1, "work"); // создание семафора "work"
    HANDLE close = CreateSemaphore(NULL, 0, 1, "close"); // создание семафора "close"
...
//1-й имя связываемого файла
    ...
    char *stringRepresentation = static_cast<char*>(memoryMap); // преобразование типа
    ...
    while (true) {
        if (readyForInput) {
            ...
            getline(cin, buffString); // ввод строки
            readyForInput = false; // установление флага
        }
        ...
        strcpy(stringRepresentation, const_cast<char*>(tempBuff.c_str())); // копирование строк с переводом из строки с++ в строку с
        ...
        if (buffString.empty()) {
            ...
            if (choice == "1") {
                ...
                ReleaseSemaphore(work, 1, NULL);
                UnmapViewOfFile(memoryMap); //отменяет отображение представления файла из адресного пространства вызывающего //процесса
